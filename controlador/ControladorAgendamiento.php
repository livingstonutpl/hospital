		...
		function create(){
			if(isset($_POST) && !empty($_POST)){
				if(isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]){
					...
					$res = Agendamiento::create($id_persona3, $id_medicoespecialidad1, $anamnesis_his, $examenfisico_his, $tratamiento_his, $fechaatencion_his, $horaatencion_his, $estado_his, $verificador_his, $verificador2_his);
					if($res){
		...
		function update(){
			if(isset($_POST) && !empty($_POST)){
				if(isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]){
					...
					$verificador_his = $id_persona3."-".$fechaatencion_his."-".$horaatencion_his;
					$verificador2_his = $id_medicoespecialidad1."-".$fechaatencion_his."-".$horaatencion_his;
					$res = Agendamiento::update($id_historiaclinica, $id_persona3, $id_medicoespecialidad1, $anamnesis_his, $examenfisico_his, $tratamiento_his, $fechaatencion_his, $horaatencion_his, $estado_his, $verificador_his, $verificador2_his);
					if($res){
		...
		function delete(){
			if(isset($_POST) && !empty($_POST)){
				if(isset($_SESSION["token"]) && $_POST["token"] == $_SESSION["token"]){
					require_once("../modelo/Agendamiento.php");
					$id_historiaclinica = Connection::sanitize($_POST["id_historiaclinica"]);
					$res = Agendamiento::delete($id_historiaclinica);
					if($res){
